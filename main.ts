...
export default class CiscoCliPlugin extends Plugin {
  // 异步加载插件
  async onload() {
  ...
  processLine(line: string, lineDiv: HTMLDivElement) {
    // 匹配命令提示符
    ...
    let currentSpan: HTMLSpanElement = createSpan(); // 初始化当前span
    let currentCls: string | null = null; // 跟踪当前span的类名
...
function createDiv({ cls }: { cls?: string } = {}) {
  const div = document.createElement('div');
  div.className = cls || 'cisco-cli-line';
  return div;
}

function createSpan({ cls }: { cls?: string } = {}) {
